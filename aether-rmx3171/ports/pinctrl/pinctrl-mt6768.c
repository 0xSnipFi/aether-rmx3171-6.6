 ...
 *
 * AETHER port from Realme 4.14.336 -> ACK 6.6
 * Adapted to 6.6 paris framework:
 *   - drive_{set,get}_direct_val -> drive_{set,get}_raw
 *   - race_free_access field dropped (no longer in mtk_pin_soc)
 *   - mtk_paris_pinctrl_probe now takes only pdev; data via of_device_id .data
 *   - mtk_eint_pm_ops_v2 replaced with mtk_paris_pinctrl_pm_ops
 */
...
MODULE_DESCRIPTION("MediaTek MT6768 Pinctrl Driver (AETHER port from 4.14)");
